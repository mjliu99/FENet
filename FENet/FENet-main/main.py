...
parser.add_argument('--lambd', type=float, default=1, help='trade-off ratio.')
    ...
    for csv_file_name in csv_file_names:
        ...
        for epoch1 in range(99):
            for index, (x, y) in enumerate(train_dataloader):
                ...
                z2 = model(x)
                if(epoch1==98):
                   th.save(x, "./x/" + csv_file_name[0:5] + ".pth")
...
###############得到训练完成的频域数据#########################
    ...
    for pth_file_name in pth_file_names:
        f=th.load("./x/"+pth_file_name)
        final=model.get_embedding(f)
        th.save(final,"./final/"+pth_file_name)
        print(pth_file_name)
# ###############得到训练完成的频域数据#########################
