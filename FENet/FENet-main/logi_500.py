...
def lg(maxacc):
    ...
    train_idx = th.load("./500/train_id.pth").to(args.device)
    test_idx = th.load("./500/test_id.pth").to(args.device)
    val_idx = th.load("./500/val_id.pth").to(args.device)

    train_embs = final_embeds[train_idx]
    test_embs = final_embeds[test_idx]
    val_embs = final_embeds[val_idx]

    labels = th.load("./500/labels.pth").to(args.device)
    ...
    for epoch in range(500):
        ...
        with th.no_grad():
            ...
            if val_acc > best_val_acc:
                ...
                eval_f1 = test_f1
                if eval_acc > maxacc:
                    torch.save(logreg.state_dict(), './500/best_model2.pth')
                best_epoch = epoch
                print(epoch)
