...
parser.add_argument('--data', type=str, default='NYU', help='data set')
parser.add_argument('--feature_size', type=int, default=176, help='feature size')
parser.add_argument('--seq_length', type=int, default=10, help='inout length')
parser.add_argument('--pre_length', type=int, default=10, help='predict length')
...
parser.add_argument('--batch_size', type=int, default=176, help='input data batch size')
...
# data_parser['NYU']['root_path']='data/NYU5094.csv'
...
for csv_file_name in csv_file_names:
    print(folder_path+csv_file_name)
    ...
    for epoch in range(args.train_epochs):
        ...
        for index, (x,y) in enumerate(train_dataloader):
            ...
            forecast,z = model(x)
            if(epoch==99):
                torch.save(z,'Zb/'+csv_file_name[0:5]+'.pth')
        ...
        print('| end of epoch {:3d} | time: {:5.2f}s | train_total_loss {:5.4f} | '.format(
...
# if __name__ == '__main__':
    ...
    #         forecast = model(x)
    #         if(epoch==99):
    #             torch.save(forecast,'Zb/50952.pth')
    #         y = y.permute(0, 2, 1).contiguous()
    #         loss = forecast_loss(forecast, y)
    #         loss.backward()
    #         my_optim.step()
