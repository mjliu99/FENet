...
def lg(maxacc):
    ...
    train_idx = th.load("./70/train_id.pth").to(args.device)
    test_idx = th.load("./70/test_id.pth").to(args.device)
    val_idx = th.load("./70/val_id.pth").to(args.device)

    train_embs = final_embeds[train_idx]
    test_embs = final_embeds[test_idx]
    val_embs = final_embeds[val_idx]

    labels = th.load("./70/labels.pth").to(args.device)
    ...
    for epoch in range(500):
        ...
        with th.no_grad():
            ...
            if val_acc > best_val_acc:
                ...
                eval_f1 = test_f1
                if eval_acc > maxacc:
                    torch.save(logreg.state_dict(), './70/best_model.pth')
                best_epoch = epoch
                print(epoch)
