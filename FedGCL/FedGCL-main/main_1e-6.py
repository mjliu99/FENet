...
parser.add_argument('--lambd', type=float, default=1e-6, help='trade-off ratio.')
    ...
    for csv_file_name in csv_file_names:
        ...
        for epoch1 in range(99):
            for index, (x, y) in enumerate(train_dataloader):
                ...
                z2 = model(x)
                if(epoch1==98):
                   th.save(x, "./x_1e-6/" + csv_file_name[0:5] + ".pth")
...
###############得到训练完成的频域数据#########################
    ...
    for pth_file_name in pth_file_names:
        f=th.load("./x_1e-6/"+pth_file_name)
        final=model.get_embedding(f)
        th.save(final,"./final_1e-6/"+pth_file_name)
        print(pth_file_name)
# ###############得到训练完成的频域数据#########################
    ...
    pth_file_names = [file_name for file_name in file_list if file_name.endswith(".pth")]
    for pth_file_name in pth_file_names:
        x = torch.load("./final_1e-6/" + pth_file_name)
        x = x.t()
        y = global_mean_pool(x, batch)
        torch.save(y, "./read_out_1e-6/" + pth_file_name)
    i = 0
    for pth_file_name in pth_file_names:
        if i == 0:
            y = torch.load("./read_out_1e-6/" + pth_file_name)
        x = torch.load("./read_out_1e-6/" + pth_file_name)
        if i != 0:
            y = torch.cat((y, x), dim=0)
        i = i + 1
    torch.save(y, "./final_1e-6.pth")
    print(args.lambd)
